// 1. Compare two objects to determine if the first contains equivalent property values to the second one.
...
// 5. Convert an array of objects to a comma-separated value (CSV) string that contains only the columns specified.
...
// 6. Target a given value in a nested JSON object based on the given key.
function getNestedValue(obj, key) {
  return key.split(".").reduce((acc, curr) => acc && acc[curr], obj);
}

// 7. Convert a specified number into an array of digits.
function numberToArray(num) {
  return Array.from(String(num), Number);
}

// 8. Filter out the specified values from a specified array. Return the original array without filtered values.
...
// 10. Extract values at specified indexes from a specified array.
